...
final class AgentChatStore {
    ...
    /// terminal state — a command lands no user record (or
    /// transformed text), so the prompt path's requestKey→record
    /// confirmation structurally cannot apply and no
    /// send.confirmed ever fires. The requestKey namespace is
    /// per-session global (shared with prompt.send; one key never
    /// collides across methods).
    ///
    /// Not an outbox entry BY DESIGN: the outbox exists because a
    /// PROMPT can be re-correlated to a committed record; a command
    /// acceptance has no record to find, so there is nothing to
    /// reconcile — the caller surfaces acceptance/error directly.
    @discardableResult
    func sendCommand(
